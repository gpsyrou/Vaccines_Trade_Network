...
-- Purpose: Gather monthly data for multiple years for the imports/exports
            of a service/commodity from https://comtrade.un.org/Data/
-- Author:  Georgios Spyrou
-- Last Updated Date: 21/08/2021
-------------------------------------------------------------------
"""
...
parser = argparse.ArgumentParser(
    description='Provided list of years to retrieve data for'
    )
...
def collect_data(
    ...
    out_folder: str) -> None:
    """
    Create a CSV file that contains the monthly data as received 
    from  https://comtrade.un.org/Data/, for a specified year.

    Args:
    ----
        api_string: String that contains the URL for the API call. 
        reporterid, reportername: Id and Name of the country of interest.
        year: Specify year of interest.
    Returns:
    -------
        The output is a .csv file that contains the data for a specified 
        country/year.
    """
    ...
    if response.status_code != 200:
        ...
        filename = f'Comtrade_Vaccines_Data_{reportername}_{year}.csv'
        filename = os.path.join(csv_by_year_out_loc, filename)
...
for api_check, reporter in enumerate(reporters_list):
    ...
    for year in args.years:
        ...
        collect_data(
            api_call_string,
            reporterid=country_id,
            reportername=countryname,
            year=year,
            out_folder=outputFilesFolder
            )
        time.sleep(6)
